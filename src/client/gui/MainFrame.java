...
public final class MainFrame extends JFrame implements Localized {
    ...
    public MainFrame(String host, int port) throws Exception {
        ...
        controller = new MainController(client, localeManager, organizations -> {
    ...
    public void updateTexts() {
        ...
        filterLabel.setText(localeManager.text("main.filter"));
    ...
    private JMenuBar menuBar() {
        ...
        darkTheme.addActionListener(action(() -> themeManager.setDark(darkTheme.isSelected(), this)));
        bar.add(language);
        bar.add(darkTheme);
        return bar;
    }
    ...
    private JButton localeButton(String title, Locale locale) {
        ...
        button.addActionListener(action(() -> localeManager.setLocale(locale)));
        return button;
    }

    private ActionListener action(Runnable runnable) {
        return event -> {
    ...
    private void bindActions() {
        ...
        add.addActionListener(action(() -> OrganizationFormDialog.show(this, localeManager, null)
                .ifPresent(organization -> controller.withOrganization("add", organization, null))));
        addIfMin.addActionListener(action(() -> OrganizationFormDialog.show(this, localeManager, null)
                .ifPresent(organization -> controller.withOrganization("add_if_min", organization, null))));
        edit.addActionListener(action(this::editSelected));
        delete.addActionListener(action(() -> selected().ifPresent(organization -> controller.simple("remove_by_id", null, String.valueOf(organization.getId())))));
        clear.addActionListener(action(this::confirmClear));
        removeFirst.addActionListener(action(() -> controller.simple("remove_first", null)));
        removeLower.addActionListener(action(() -> OrganizationFormDialog.show(this, localeManager, null)
    ...
    private void editSelected() {
        selected().ifPresent(organization -> {
            if (!controller.username().equals(organization.getOwnerUsername())) {
                showError(localeManager.text("error.owner"));
                return;
            }
    ...
    private Optional<Organization> selected() {
        ...
        if (row < 0) {
            showError(localeManager.text("error.select"));
    ...
    private long selectedId() {
        int row = table.getSelectedRow();
        return row < 0 ? -1 : tableModel.organizationAt(row).getId();
    }

    private void restoreSelection(long id) {
        if (id < 0) return;
        int row = tableModel.rowById(id);
        if (row >= 0) table.setRowSelectionInterval(row, row);
    }

    private void showError(String message) {
        JOptionPane.showMessageDialog(this, message, localeManager.text("app.title"), JOptionPane.ERROR_MESSAGE);
    }

    private void showMessage(String message, String title) {
        JOptionPane.showMessageDialog(this, localeManager.message(message), title, JOptionPane.INFORMATION_MESSAGE);
    }
